...
def index():
    ...
    pagination = query.order_by(Post.timestamp.desc()).paginate(
        page, per_page=current_app.config['HONGSH_POSTS_PER_PAGE'],
        error_out=False)
...
def post(id):
    ...
    if page == -1:
        page = (post.comments.count() - 1) // \
            current_app.config['HONGSH_COMMENTS_PER_PAGE'] + 1
    pagination = post.comments.order_by(Comment.timestamp.asc()).paginate(
        page, per_page=current_app.config['HONGSH_COMMENTS_PER_PAGE'],
        error_out=False)
...
def followers(username):
    ...
    pagination = user.followers.paginate(
        page, per_page=current_app.config['HONGSH_FOLLOWERS_PER_PAGE'],
        error_out=False)
